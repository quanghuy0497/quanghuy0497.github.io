...
description: A gentle introduction to Bayesian networks and probabilistic graphical models.
categories: mathematic
tags: bayesian probability
thumbnail: assets/img/bayesnet-thumb.png
date: 2022-11-29
---
...
**A Bayesian network** is a *probabilistic graphical model* that represents the causal probabilistic relationship among a set of random variables, their conditional dependencies, and it provides a compact ***representation of a joint probability distribution***. Bayesian networks are ideal for taking an event that occurred and predicting the likelihood that any one of several possible known causes was the contributing factor.

Formally, Bayesian networks are acyclic directed graphs (a directed graph with no directed cycles) *whose nodes represent variables in the Bayesian sense*. If there exists a causal probabilistic dependence between two random variables in the graph, the corresponding two nodes are connected by a ***directed edge***, while the directed edge from a node $$A$$ to node $$B$$ indicates the random variable $$A$$ causes the random variable $$B$$, represented by the *conditional probability* $$P(B\mid A)$$.
...
Two events can cause Grass to be wet: an active Sprinkler or Rain. This situation can be modeled with a Bayesian network. Each variable has two possible values: True $$T$$ or False $$F$$. 
...
- The model can answer question about the presence of a cause given the presence of an effect like *“What is the probability that it is raining, given the grass it wet?”* $$P(R_T\mid G_T)$$. By using the conditional probability formula and summing over all nuisance variables, we have:
...
- Based on the conditional probability tables on the diagram, we can calculate individual conditional probability value, for example:
...
- Therefore, we have:
    ...
    $$
    \begin{aligned}
    P(R_T|G_T) &= \frac{0.00198_{TTT} + 0.1584_{TFT}}{0.00198_{TTT}+0.288_{TFF}+0.1584_{TFT} + 0.0_{FFF}} \approx 35.77\% 
    \end{aligned}
    $$
