...
description: A brief introduction to the multi-objective optimization problem, Pareto fronts, scalarization methods, and hypernetworks for Pareto set learning.
categories: mathematic
tags: optimization
thumbnail: assets/img/moo-thumb.png
date: 2023-07-12
---
...
\begin{equation}
\begin{aligned}
\boldsymbol x^* &= \underset{x}{\operatorname{argmin}} \boldsymbol f(x)\\
\boldsymbol f(x) &=\{f_1(x), \ f_2(x),\cdots, \ f_m(x) \}
\end{aligned}
\end{equation}
$$
...
- Let $$x_1,\ x_2 \in \mathcal X$$, $$x_1$$ is said to **dominate** $$x_2$$, denoted as $$x_1 \boldsymbol\prec x_2$$, if and only if $$\forall i \in \{1, \cdots, m\}, \ f_i(x_1) \leq f_i(x_2)$$ and $$\exists i \in \{1, \cdots, m\}, \ f_i(x_1) < f_i(x_2)$$, i.e. for all criteria $$x_1$$ is at least as good as $$x_2$$, but there’s at least one criteria at which $$x_1$$ is strictly better than $$x_2$$
    - In addition, $$x_1$$ is said to **strictly dominate** $$x_2$$, denoted as $$x_1 \boldsymbol\prec_{strict} x_2$$, if and only if $$\forall i \in \{1, \cdots, m\}, \ f_i(x_1) < f_i(x_2)$$
- A solution $$\boldsymbol x^* \in \mathcal X$$ is a **Pareto optimal** if  $$\nexists \hat x \in \mathcal X, \ \hat x \boldsymbol{\prec x}^*$$
    - A solution $$x' \in \mathcal X$$ is a **weakly Pareto optimal** if there is no $$\nexists \hat x \in \mathcal X, \ \hat x \boldsymbol\prec_{strict} x'$$
- The set of all Pareto optimal solutions $$P = \{\boldsymbol x*\} \subseteq \mathcal X$$ is called the **Pareto Set**, and its images on the objective space $$\boldsymbol f(P) = \{\boldsymbol f(x)\mid x  \in P\}$$ is called the **Pareto Front**. The shape of the Pareto surface indicates the nature of the trade-off between the different objective functions
    - Similarly, we can define the **weakly Pareto set** $$P_{weak}$$ and weak Pareto front $$\boldsymbol f(P_{weak})$$
...
A common approach for solving the multi-objective optimization (MOO) problem is transforming multi-objective problem into a single objective optimization (SOO) problem. This can be done with a **scalarization** functions, parameterized by a **preference vector** $$\boldsymbol \lambda \in \Lambda = \{\boldsymbol \lambda \in \mathbb R^m_+ \mid \sum \lambda_i = 1 \}$$, to transform MOO to SOO, that is $$\mathfrak s_{\boldsymbol\lambda} (f(x)): \mathbb R^m \rightarrow \mathbb R$$. Each $$\boldsymbol \lambda$$ represent a different trade-off over objectives, attaining to a single Pareto solution. Under the scalarization technique, we now consider the MOO problem as *single objective optimization*. By optimizing $$\mathfrak s_\lambda$$, we can obtain a corresponding Pareto solution:

$$
\begin{equation}
\boldsymbol x^*_{\boldsymbol\lambda} = \underset{\boldsymbol x \in \mathcal X}{\operatorname{argmin}} \ \mathfrak s_{\boldsymbol\lambda}(f(x))
\end{equation}
$$
...
- **Weighted sum**:
    ...
    $$
    \begin{equation}
    \mathfrak s_{\boldsymbol\lambda}(f(x)) = \sum_{i=1}^m \lambda_i f_i(x)
    \end{equation}
    $$
...
- **Weighted exponential sum**:
    ...
    $$
    \begin{equation}
    \mathfrak s_{\boldsymbol\lambda}(f(x)) = \sum_{i=1}^m \lambda_i [f_i(x)]^p
    \end{equation}
    $$
...
- **Weighted Tchebycheff**:
    ...
    $$
    \begin{equation}
    \mathfrak s_{\boldsymbol\lambda}(f(x)) = \underset{1 \leq i \leq m}{\max}\{\lambda_i \lvert f_i(x) - (z^*_i + \epsilon)\rvert\}
    \end{equation}
    $$
    
    where $$\boldsymbol z^* = (z^*_1, \cdots, z^*_m)$$ is the ideal value for the objective functions $$\boldsymbol f(x)$$, i.e., the lower-bound for minimization problem, and $$\epsilon > 0$$ is a small positive scalar, and $$(z^*_i + \epsilon)$$ is so called the (unachievable) utopia value for the $$i$$-th objective $$f_i(x)$$.
    ...
    By solving the Tchebycheff scalarization function, we can obtain all Pareto solutions $$\boldsymbol x \in P$$. However, some solutions might be weakly Pareto solution.
